...
describe('Basic', () => {
  ...
  test('Create and Find Shop', async () => {
    ...
    let foundShop1ById = await models.Shop.findOne( (stmt, s) => stmt.where(s.prop({id: shop1.id})))
    let foundShop1ByLocation = await models.Shop.findOne( (stmt, s) => stmt.where(s.prop({location: expectedShop1.location})))

    expect(foundShop1ById).toMatchObject(foundShop1ByLocation)
    expect(foundShop1ById).toMatchObject(expectedShop1)

    let foundShop2ById = await models.Shop.findOne( (stmt, s) => stmt.where(s.prop({id: shop2.id})))
    expect(foundShop2ById).toMatchObject(expectedShop2)

    let foundShopNotExists =  await models.Shop.findOne( (stmt, s) => stmt.where(s.prop({id: 100000})))
    ...
    let foundProductsByShopId1 = await models.Product.find( (stmt, s) => stmt.where(s.prop({shopId: shop1.id})) )
    ...
    let foundProductsByShopId2 = await models.Product.find( (stmt, s) => stmt.where(s.prop({shopId: shop2.id})) )
